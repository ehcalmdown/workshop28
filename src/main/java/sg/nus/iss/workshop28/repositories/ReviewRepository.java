package sg.nus.iss.workshop28.repositories;
...
public class ReviewRepository {
     ...
     */
    public Optional<Game> aggregrateGameReviews(String gameId) {//aggregate is to take items that match criteria and then group them together according to said criteria, in this case, the game id
        MatchOperation matchGameId = Aggregation.match(
                Criteria.where("gid").is(Integer.parseInt(gameId)));


                //lookup all fields presented that fulfil criteria (local is from model and json, foreign is from html)"
    ...
    //Returns a list of all the games with the highest rating given by a user. 
    public List<Comment> aggregateGamesComment(Integer limit, String username, Integer rating) {//using aggregate to list games by rating
        ...
        //endpoint should return similar as shown:
        // {
